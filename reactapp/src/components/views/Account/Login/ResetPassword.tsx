...
let ResetPassword = () => {
    ...
    let handleReset = (e: any) => {
        e.preventDefault();
        setLoading(true);
        fetch(`${server}/account/resetpassword`, {
            method: 'POST',
            headers: {
            ...
            .then((response) => {
                ...
                if (contentType && contentType.indexOf("application/json") !== -1) {
                    ...
                    throw new TypeError("Could not find Email.");
                }
            })
            .then((data) => {
                setHasToken(true);
    ...
    const handleChange = async (e: any) => {
        e.preventDefault();
        fetch(`${server}/account/changepassword`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        flex items-center justify-center">
                ...
                <div className="w-full h-100">
                    {(success || errorMessage) && (
                        <div className={`w-full px-4 py-3 rounded-lg mt-2 text-gray-200 border ${success ? 'bg-green-600' : errorMessage ? 'bg-red-600' : ''}`}>
                        ...
                        {hasToken && (
                            <>
                                <div>
                                    ...
                                    <input disabled={isLoading} value={form.token} onChange={handleInputChange} type="text" name="token" id="token" placeholder="Enter Reset-Token" className="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-blue-500 focus:bg-white focus:outline-none" autoFocus autoComplete='' required />
                                </div>
                                <div>
                                    <label className="block text-gray-700">New Password</label>
                                    <input disabled={isLoading} value={form.password} onChange={handleInputChange} type="password" name="password" id="password" placeholder="Enter a new Password" className="w-full px-4 py-3 rounded-lg bg-gray-200 mt-2 border focus:border-blue-500 focus:bg-white focus:outline-none" autoFocus autoComplete='' required />
                                </div>
                            </>
                        )}
                        ...
                        <div className="text-right mt-2">
                            {hasToken ? (
                                <button onClick={(e) => {
                                    e.preventDefault();
                                    setHasToken(false);
                                }} className="text-sm font-semibold text-gray-700 hover:text-blue-700 focus:text-blue-700">Need a token?</button>
                            ) : (
                                <button onClick={(e) => {
                                    e.preventDefault();
                                    setHasToken(true);
                                }} className="text-sm font-semibold text-gray-700 hover:text-blue-700 focus:text-blue-700">Already have a token?</button>
                            )}
                        </div>
